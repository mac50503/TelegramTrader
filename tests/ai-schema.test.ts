...
describe("respuesta estructurada del agente", () => {
  it("conserva y ordena ambos extremos del rango", () => {
    expect(signalAnalysisSchema.parse({ isSignal: true, symbol: "XAUUSD", side: "BUY", entryMin: 4654, entryMax: 4650,
      stopLoss: 4645, takeProfits: [4660], confidence: 0.98 })).toMatchObject({ entry: "4650", entryMin: "4650", entryMax: "4654" });
  });
  it("acepta una señal JSON válida", () => {
    expect(signalAnalysisSchema.parse({ isSignal: true, symbol: "XAUUSD", side: "BUY", entry: 3345, stopLoss: 3335, takeProfits: [3370], confidence: 0.98 }))
