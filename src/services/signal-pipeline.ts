...
export class SignalPipeline {
  ...
  async ingest(message: TelegramMessage): Promise<TradeSignal | null> {
    ...
    logEvent(this.logger, "TELEGRAM_MESSAGE_RECEIVED", { signalId: signal.id, source: message.source, status: "RECEIVED" });
    this.audit.recordEvent("TELEGRAM_MESSAGE_RECEIVED", { signalId: signal.id, source: message.source, status: "RECEIVED" });
    try {
      this.signals.setStatus(signal.id, "ANALYZING");
      logEvent(this.logger, "SIGNAL_ANALYSIS_STARTED", { signalId: signal.id, source: message.source, status: "ANALYZING" });
      ...
      logEvent(this.logger, "SIGNAL_DETECTED", { signalId: signal.id, source: message.source, status: "ANALYZING" });
      this.audit.recordEvent("SIGNAL_DETECTED", { signalId: signal.id, source: message.source, status: "ANALYZING" });
      ...
      logEvent(this.logger, "SIGNAL_VALIDATED", { signalId: signal.id, source: message.source, status: "VALIDATED" });
  ...
  private applyRiskAndQueue(signalId: string, context: Mt5Context): void {
    ...
    logEvent(this.logger, "SIGNAL_QUEUED", { signalId: signal.id, source: signal.source, status: "QUEUED", volume: decision.volume });
    this.audit.recordEvent("SIGNAL_QUEUED", { signalId: signal.id, source: signal.source, status: "QUEUED", payload: { volume: decision.volume } });
  }
  ...
  private reject(signal: TradeSignal, code: string, reason: string): TradeSignal {
    ...
    logEvent(this.logger, "SIGNAL_REJECTED", { signalId: signal.id, source: signal.source, status: "REJECTED", code, reason });
    this.audit.recordEvent("SIGNAL_REJECTED", { signalId: signal.id, source: signal.source, status: "REJECTED", payload: { code, reason } });
    return this.signals.findById(signal.id)!;
  }
}
